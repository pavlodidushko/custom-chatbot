<script lang="ts">
  ...
  import CustomChatbot from "./lib/CustomChatbot.svelte";
  ...
  const routes = {
    ...
    "/chat/:chatId": wrap({
      component: Chat,
      conditions: (detail) => {
        const chatId = parseInt(detail?.params?.chatId as string);
        return $chatsStorage.find((chat) => chat.id === chatId) !== undefined;
      },
    }),
  ...
  // Execute handleQueryParams on component mount
...
<svelte:window on:keydown={(e) => checkModalEsc(e)} />
