<script context="module" lang="ts">
  ...
  import {
    getChatSettingObjectByKey,
    getGlobalSettingObjectByKey,
    getChatDefaults,
    getExcludeFromProfile,
    chatSortOptions,
    globalDefaults,
  ...
  export const latestModelMap = persisted(
    "latestModelMap",
    {} as Record<Model, Model>,
  ); // What was returned when a model was requested
  ...
  const customchat = [
    "Newsletter Checker",
    "Recipe and Puzzle Checker",
    "This review Checker",
  ];
  export const addCustomChat = (
    profile: ChatSettings | undefined = undefined,
  ) => {
    console.log("addCustomChat");
    const chats = get(chatsStorage);
    const customchatId = chats.findIndex((chat) => chat.id == -1);

    if (customchatId == -1) {
      for (let i = -3; i <= -1; i++) {
        profile = JSON.parse(
          JSON.stringify(profile || getProfile("")),
        ) as ChatSettings;
        const nameMap = chats.reduce((a, chat) => {
          a[chat.name] = chat;
          return a;
        }, {});
        ...
        chats.push({
          id: i,
          name: newName(`${customchat[i + 3]}`, nameMap),
          settings: profile,
          messages: [],
  ...
  export const updateChatSettings = (chatId: number) => {
    ...
    if (chat.sessionStarted === undefined)
      chat.sessionStarted = !!chat.messages.find((m) => m.role === "user");
    chatsStorage.set(chats);
  };
  ...
  export const updateRunningTotal = (
    chatId: number,
    usage: Usage,
    model: Model,
  ) => {
  ...
  export const subtractRunningTotal = (
    chatId: number,
    usage: Usage,
    model: Model,
  ) => {
  ...
  export const getMessages = (chatId: number): Message[] => {
    ...
    console.log(chatId, currentChatMessages);
  ...
  export const insertMessages = (
    ...
    setMessages(
      chatId,
      messages.filter((m) => true),
    );
  };

  export const deleteSummaryMessage = (chatId: number, uuid: string) => {
    const message = getMessage(chatId, uuid);
    if (message && message.summarized)
      throw new Error("Unable to delete summarized message");
    if (message && message.summary) {
      // messages we summarized
  ...
  export const deleteMessage = (chatId: number, uuid: string) => {
    ...
    if (message?.summarized)
      throw new Error("Unable to delete summarized message");
    if (message?.summary)
      throw new Error("Unable to directly delete message summary");
    if (index < 0) {
  ...
  export const truncateFromMessage = (chatId: number, uuid: string) => {
    ...
    const message = getMessage(chatId, uuid);
    if (message && message.summarized)
      throw new Error("Unable to truncate from a summarized message");
    if (index < 0) {
    ...
    switch (type) {
      ...
      case "boolean":
        ...
        return (
          value === "true" ||
          value === "yes" ||
          (value ? value !== "false" && value !== "no" && !!value : false)
        );
      default:
        return value;
    }
  ...
  export const setChatSettingValueByKey = (
    chatId: number,
    ...
    if (d === null || d === undefined) {
      throw new Error(
        'Unable to determine setting type for "' +
          key +
          ' from default of "' +
          d +
          '"',
  ...
  export const setChatSettingValue = (
    chatId: number,
    setting: ChatSetting,
    value,
  ) => {
  ...
  export const getChatSettingValueNullDefault = (
    chatId: number,
    setting: ChatSetting,
  ): any => {
    const chats = get(chatsStorage);
  ...
  export const setGlobalSettingValueByKey = (
  ...
  export const getGlobalSettingValue = (
    key: keyof GlobalSetting,
    value,
  ): any => {
    const store = get(globalStorage);
    return store[key];
  ...
  export const saveCustomProfile = (profile: ChatSettings) => {
    ...
    const mt =
      profile.profileName && profile.profileName.trim().toLocaleLowerCase();
    const sameTitle = Object.values(profiles).find(
      (c) =>
        c.profile !== profile.profile &&
        c.profileName &&
        c.profileName.trim().toLocaleLowerCase() === mt,
    );
    if (sameTitle) {
      throw new Error(
        `Sorry, another profile already exists with the name "${profile.profileName}"`,
      );
    }
    if (!mt) {
      throw new Error(
        "Sorry, you need to enter a valid name for your profile.",
      );
    }
  ...
  export const getChatSortOption = (): ChatSortOption => {
    const store = get(globalStorage);
    return (
      chatSortOptions[store.chatSort] ||
      chatSortOptions[globalDefaults.chatSort]
    );
  };

  export const setChatSortOption = (sortName: any) => {
    const store = get(globalStorage);
    store.chatSort = chatSortOptions[sortName]
      ? sortName
      : globalDefaults.chatSort;
    globalStorage.set(store);
  ...
  export const setLatestKnownModel = (
