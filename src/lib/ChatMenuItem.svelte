...
<li>
  ...
  {:else}
    <a
      href={`#/chat/${chat.id < 0 ? `customgpt/${chat.id}` : chat.id}`}
    ...
    >
      {#if chat.id >= 0}
        {#if waitingForConfirm}
          <a
            class="is-pulled-right is-hidden px-1 py-0 has-text-weight-bold delete-button"
            href={"$"}
            on:click|preventDefault={() => delChat()}
            ><Fa icon={faCircleCheck} /></a
          >
        {:else}
          <a
            class="is-pulled-right is-hidden px-1 py-0 has-text-weight-bold edit-button"
            href={"$"}
            on:click|preventDefault={() => edit()}><Fa icon={faPencil} /></a
          >
          <a
            class="is-pulled-right is-hidden px-1 py-0 has-text-weight-bold delete-button"
            href={"$"}
            on:click|preventDefault={() => delChat()}><Fa icon={faTrash} /></a
          >
        {/if}
      {/if}
      <span class="chat-item-name"
        >{#if chat.id >= 0}
          <Fa class="mr-2 chat-icon" size="xs" icon={faMessage} />
        {/if}{chat.name || `Chat ${chat.id}`}</span
      >
    </a>
  {/if}
</li>
