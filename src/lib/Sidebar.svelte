<script lang="ts">
  ...
  import ChatbotMenuItem from "./ChatbotMenuItem.svelte";

  $: sortedChats = $chatsStorage.sort(getChatSortOption().sortFn);
  $: chatbots = $chatsStorage.sort(getChatSortOption().sortFn);
  $: activeChatId =
  ...
  let chatbotMenus = [
    { id: -3, name: "Newsletter Checker" },
    // { id: -2, name: "Recipe and Puzzle Checker" },
    // { id: -1, name: "Inside This Issue Review" },
  ];
...
>
  <div class="menu-expanse">
    ...
    <ul class="menu-list mb-3">
      {#each chatbotMenus as botmenuitem, i}
        {#key i}
          <ChatbotMenuItem {botmenuitem} />
        {/key}
      {/each}
    </ul>
