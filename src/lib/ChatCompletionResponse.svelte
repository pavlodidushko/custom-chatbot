  ...
  export class ChatCompletionResponse {
    constructor(opts: ChatCompletionOpts) {
      ...
      if (opts.fillMessage) {
        ...
        this.offsetTotals =
          opts.fillMessage.usage &&
          JSON.parse(JSON.stringify(opts.fillMessage.usage));
        this.isFill = true;
      }
      if (opts.onMessageChange)
        this.messageChangeListeners.push(opts.onMessageChange);
    }
    ...
    async updateImageFromSyncResponse(
      images: string[],
      prompt: string,
      model: Model,
    ) {
    ...
    updateFromSyncResponse(response: Response) {
      ...
      if (!response.choices) {
        return this.updateFromError(
          response?.error?.message || "unexpected response from API",
        );
      }
      response.choices?.forEach((choice, i) => {
        ...
        if (exitingMessage) {
          message.content = this.initialFillMerge(
            message.content,
          ...
          message.usage.prompt_tokens =
            ...
            (this.offsetTotals?.prompt_tokens || 0);
          message.usage.total_tokens =
            (response?.usage?.total_tokens || 0) +
            (this.offsetTotals?.total_tokens || 0);
        } else {
          message.content = choice.message.content;
    ...
    updateFromAsyncResponse(response: Response) {
      ...
      if (!response.choices || response?.error) {
        return this.updateFromError(
          response?.error?.message || "unexpected streaming response from API",
        );
      }
      response.choices?.forEach((choice, i) => {
        const message =
      ...
      // total up the tokens
      const promptTokens =
        this.promptTokenCount + (this.offsetTotals?.prompt_tokens || 0);
    ...
    updateFromClose(force: boolean = false): void {
      if (
        !this.finished &&
        !this.error &&
        !this.messages?.find((m) => m.content)
      ) {
        if (!force)
